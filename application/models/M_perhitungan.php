...
class M_perhitungan extends CI_Model {
    ...
    public function getAllProducts($filter = []){
        ...
        if(isset($filter['nama']) && !empty($filter['nama']) && $filter['nama'] !== "0"){
            $this->db->where('sunscreen.namaProduk', $filter['nama']);
        }
        
        if(isset($filter['idJenisKulit']) && !empty($filter['idJenisKulit']) && $filter['idJenisKulit'] !== "0"){
            $this->db->where_in('sunscreen.idJenisKulit', [1, $filter['idJenisKulit']]);
        }
        
        if(isset($filter['idAsalBrand']) && !empty($filter['idAsalBrand']) && $filter['idAsalBrand'] !== "0"){
            $this->db->where('sunscreen.idAsalBrand', $filter['idAsalBrand']);
        }
        
        if(isset($filter['harga']) && !empty($filter['harga']) && $filter['harga'] !== "0"){
            if($filter['harga'] == "400000"){
                $this->db->where('sunscreen.harga >', 400000);
            }else{
                $harga = explode(",", $filter['harga']);
                $this->db->where('sunscreen.harga >=', $harga[0]);
                $this->db->where('sunscreen.harga <=', $harga[1]);
            }
        }
        
        if(isset($filter['spf']) && !empty($filter['spf']) && $filter['spf'] != 0){
            $spf = explode(",", $filter['spf']);
            $this->db->where('sunscreen.spf >=', $spf[0]);
            $this->db->where('sunscreen.spf <=', $spf[1]);
        }
        
        if(isset($filter['protectionGrade']) && !empty($filter['protectionGrade']) && $filter['protectionGrade'] !== "0"){
        ...
        if(!empty($query)){
            foreach($query as $key => $val){
                ...
                $raw[$val->idSunscreen]['id'] = $val->idSunscreen;
                $raw[$val->idSunscreen]['name'] = $val->namaProduk;
                $raw[$val->idSunscreen]['Price'] = (float) $val->harga;
                $raw[$val->idSunscreen]['SPF'] = (float) $val->spf;
                $raw[$val->idSunscreen]['Protection Grade'] = cvtPG($val->protectionGrade);
                $raw[$val->idSunscreen]['Rating'] = (float) $val->ratingProduk;
                $raw[$val->idSunscreen]['Berat'] = (float) $val->berat;
                $raw[$val->idSunscreen]['Users Recommend'] = (float) $val->usersRecommend;
                $raw[$val->idSunscreen]['Users Repurchase'] = (float) $val->usersRepurchase;
                $raw[$val->idSunscreen]['link'] = is_null($val->link) ? "https://www.sociolla.com/140-skin-care" : $val->link;
            }
        }
        // ej($arr);
        return [
    ...
    function getRiwayatPerhitungan(){
        ...
        ->order_by('perhitungan.created_at', 'desc');
